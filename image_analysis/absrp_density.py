...
class atomnumanalysis:
    def __init__(self, fname, gname, detuning):
        ...
        atom_num_summing, atom_num, density_array = self.readhdf(fname, gname, param, detuning)
        print(atom_num)
        # atom_num = np.random.rand(10)+3

        parameter = np.linspace(-10,0, 11)[1:len(atom_num)]
        #parameter = np.linspace(5,15, 11)[1:len(atom_num)]

        mpl.style.use("seaborn")
        self.fig, self.ax = plt.subplots()
        self.plot(parameter, density_array[1:], param=param)
    ...
    def readhdf(self, fname, gname, param, detuning):
        with h5py.File(fname, "r") as f:
            ...
            for subg in group.keys(): #Cycle through subfolders (ie TOF expansion)
                ...
                img_data = np.divide(np.array(group[subg][test[0]]), np.array(group[subg][test[1]])) #Signal / Background
                ...
                print("Atom number summing: " + str(atom_num_summing/1e6))
                ...
                density_array.append(density[0])
                amplitude_array.append(fitresult["amp"])
                print('Density:' + str(density[0]))
                #atom_num = np.append(atom_num, sc*(param["pixeltomm"]**2)/cross_section)

        return atom_num_summing, atom_num, density_array

    def plot(self, parameter, atom_num, param={}):
        color = 'C1'
        self.ax.plot(parameter, atom_num, marker='o', mfc=color, markeredgewidth=0.8, markeredgecolor='k', linestyle='')
...
gname = "Density_20240304_" + "19" + "1745"
detuning = 0 # in MHz
